...
{
    // printk("syscall in fn: %d\n", fn);
    syscall[fn](arg1, arg2, arg3);
    // printk("syscall out fn: %d\n", fn);
}
...
void sys_kill(pid_t id)
{
    invoke_syscall(SYSCALL_KILL, id, IGNORE, IGNORE);
}

void sys_waitpid(pid_t id)
{
    invoke_syscall(SYSCALL_WAIT, id, IGNORE, IGNORE);
}
...
{
    // printk("move cursor three args: %d %d %d\n", SYSCALL_CURSOR, x, y);
...
{
    invoke_syscall(SYSCALL_SEM_INIT, (int)sem, val, IGNORE);
}
void semaphore_up(semaphore_t *sem)
{
    invoke_syscall(SYSCALL_SEM_UP, (int)sem, IGNORE, IGNORE);
}

void semaphore_down(semaphore_t *sem)
{
    invoke_syscall(SYSCALL_SEM_DOWN, (int)sem, IGNORE, IGNORE);
}

void barrier_init(barrier_t *barrier, int val)
{
    invoke_syscall(SYSCALL_BARRIER_INIT, (int)barrier, val, IGNORE);
}

void barrier_wait(barrier_t *barrier)
{
    invoke_syscall(SYSCALL_BARRIER_WATI, (int)barrier, IGNORE, IGNORE);
}
